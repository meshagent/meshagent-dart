...
class Loader {
  ...
  factory Loader(String source, {Uri? sourceUrl, bool recover = false, ErrorListener? errorListener}) {
    var parser = Parser(source, sourceUrl: sourceUrl, recover: recover, errorListener: errorListener);
  ...
  YamlScalar _parseByTag(ScalarEvent scalar) {
    switch (scalar.tag) {
      ...
      default:
        if (scalar.tag != null && scalar.tag!.startsWith('!') && scalar.tag != '!') {
          var taggedValue = scalar.value.isEmpty ? scalar.tag! : '${scalar.tag} ${scalar.value}';
          return YamlScalar.internal(taggedValue, scalar);
        }
        return YamlScalar.internal(scalar.value, scalar);
    }
  }
  ...
  YamlScalar _parseScalar(ScalarEvent scalar) => _tryParseScalar(scalar) ?? YamlScalar.internal(scalar.value, scalar);
  ...
  YamlScalar? _parseNull(ScalarEvent scalar) => switch (scalar.value) {
    '' || 'null' || 'Null' || 'NULL' || '~' => YamlScalar.internal(null, scalar),
    _ => null,
  };
  ...
  ///
  /// Returns `null` if parsing fails.
  YamlScalar? _parseNumber(ScalarEvent scalar, {bool allowInt = true, bool allowFloat = true}) {
    var value = _parseNumberValue(scalar.value, allowInt: allowInt, allowFloat: allowFloat);
  ...
  num? _parseNumberValue(String contents, {bool allowInt = true, bool allowFloat = true}) {
    ...
    if ((firstChar >= $0 && firstChar <= $9) || ((firstChar == $plus || firstChar == $minus) && secondChar >= $0 && secondChar <= $9)) {
    ...
    if ((firstChar == $dot && secondChar >= $0 && secondChar <= $9) || (firstChar == $minus || firstChar == $plus) && secondChar == $dot) {
