...
class Scanner {
  ...
  bool get _isDocumentIndicator => _scanner.column == 0 && _isBlankOrEndAt(3) && (_scanner.matches('---') || _scanner.matches('...'));
  ...
  void _fetchMoreTokens() {
    while (true) {
      if (_tokens.isNotEmpty) {
        ...
        if (!_simpleKeys.any((key) => key != null && key.tokenNumber == _tokensParsed)) {
          break;
        }
      }
      ...
      case COLON:
        if (!_inBlockContext && _tokens.isNotEmpty) {
          ...
          if (token.type == TokenType.flowSequenceEnd ||
              ...
              (token.type == TokenType.scalar && (token as ScalarToken).style.isQuoted)) {
            _fetchValue();
            return;
          }
        }
  ...
  void _invalidScalarCharacter() => _scanner.error('Unexpected character.', length: 1);
  ...
  void _staleSimpleKeys() {
    for (var i = 0; i < _simpleKeys.length; i++) {
      ...
      if (key.required) {
        ...
        _tokens.insert(key.tokenNumber - _tokensParsed, Token(TokenType.key, key.location.pointSpan() as FileSpan));
      }
  ...
  void _removeSimpleKey() {
    ...
    if (key != null && key.required) {
      throw YamlException("Could not find expected ':' for simple key.", key.location.pointSpan());
    }
  ...
  void _rollIndent(int column, TokenType type, SourceLocation location, {int? tokenNumber}) {
    if (!_inBlockContext) return;
  ...
  void _fetchBlockEntry() {
    if (_inBlockContext) {
      if (!_simpleKeyAllowed) {
        throw YamlException('Block sequence entries are not allowed here.', _scanner.emptySpan);
      }

      _rollIndent(_scanner.column, TokenType.blockSequenceStart, _scanner.location);
    } else {
  ...
  void _fetchKey() {
    if (_inBlockContext) {
      if (!_simpleKeyAllowed) {
        throw YamlException('Mapping keys are not allowed here.', _scanner.emptySpan);
      }

      _rollIndent(_scanner.column, TokenType.blockMappingStart, _scanner.location);
    }
  ...
  void _fetchValue() {
    ...
    if (simpleKey != null) {
      ...
      _tokens.insert(simpleKey.tokenNumber - _tokensParsed, Token(TokenType.key, simpleKey.location.pointSpan() as FileSpan));

      // In the block context, we may need to add the
      // [TokenType.BLOCK_MAPPING_START] token.
      _rollIndent(simpleKey.column, TokenType.blockMappingStart, simpleKey.location, tokenNumber: simpleKey.tokenNumber);
  ...
  void _scanToNextToken() {
    ...
    while (true) {
      ...
      while (_scanner.peekChar() == SP || ((!_inBlockContext || !afterLineBreak) && _scanner.peekChar() == TAB)) {
        _scanner.readChar();
      }

      if (_scanner.peekChar() == TAB) {
        _scanner.error('Tab characters are not allowed as indentation.', length: 1);
      }
  ...
  Token? _scanDirective() {
    ...
    if (!_isBreakOrEnd) {
      throw YamlException('Expected comment or line break after directive.', _scanner.spanFrom(start));
    }
  ...
  String _scanDirectiveName() {
    ...
    if (name.isEmpty) {
      ...
      throw YamlException('Unexpected character in directive name.', _scanner.emptySpan);
    }
  ...
  Token _scanAnchor({bool anchor = true}) {
    ...
    if (name.isEmpty ||
        (!_isBlankOrEnd &&
            ...
            next != AT &&
            next != GRAVE_ACCENT)) {
      throw YamlException('Expected alphanumeric character.', _scanner.emptySpan);
    }
  ...
  String _scanTagUri({String? head, bool flowSeparators = true}) {
    ...
    var start = _scanner.position;
    var char = _scanner.peekChar();
    while (_isTagChar || (flowSeparators && (char == COMMA || char == LEFT_SQUARE || char == RIGHT_SQUARE))) {
      _scanner.readChar();
  ...
  Token _scanBlockScalar({bool literal = false}) {
    ...
    if (char == PLUS || char == HYPHEN) {
      ...
      if (_isDigit) {
        ...
        if (_scanner.peekChar() == NUMBER_0) {
          throw YamlException('0 may not be used as an indentation indicator.', _scanner.spanFrom(start));
        }
    ...
    if (!_isBreakOrEnd) {
      throw YamlException('Expected comment or line break.', _scanner.emptySpan);
    }
    ...
    while (_scanner.column == indent && !_scanner.isDone) {
      ...
      if (!literal && leadingBreak.isNotEmpty && !leadingBlank && !trailingBlank) {
    ...
    return ScalarToken(_scanner.spanFrom(start, end), buffer.toString(), literal ? ScalarStyle.LITERAL : ScalarStyle.FOLDED);
  }
  ...
  ({int indent, String trailingBreaks}) _scanBlockScalarBreaks(int indent) {
    ...
    while (true) {
      while ((indent == 0 || _scanner.column < indent) && _scanner.peekChar() == SP) {
        _scanner.readChar();
      }
  ...
  Token _scanFlowScalar({bool singleQuote = false}) {
    ...
    while (true) {
      ...
      while (!_isBlankOrEnd) {
        var char = _scanner.peekChar();
        if (singleQuote && char == SINGLE_QUOTE && _scanner.peekChar(1) == SINGLE_QUOTE) {
          ...
          switch (_scanner.peekChar(1)) {
            ...
            case LETTER_CAP_U:
              ...
              break;
            default:
              throw YamlException('Unknown escape character.', _scanner.spanFrom(escapeStart));
          }
          ...
          if (codeLength != null) {
            ...
            for (var i = 0; i < codeLength; i++) {
              if (!_isHex) {
                _scanner.readChar();
                throw YamlException('Expected $codeLength-digit hexidecimal number.', _scanner.spanFrom(escapeStart));
              }
            ...
            if ((value >= 0xD800 && value <= 0xDFFF) || value > 0x10FFFF) {
              throw YamlException('Invalid Unicode character escape code.', _scanner.spanFrom(escapeStart));
            }
    ...
    return ScalarToken(_scanner.spanFrom(start), buffer.toString(), singleQuote ? ScalarStyle.SINGLE_QUOTED : ScalarStyle.DOUBLE_QUOTED);
  }
  ...
  Token _scanPlainScalar() {
    ...
    while (true) {
      ...
      while (_isBlank || _isBreak) {
        if (_isBlank) {
          ...
          if (leadingBreak.isNotEmpty && _scanner.column < indent && _scanner.peekChar() == TAB) {
    ...
    return ScalarToken(_scanner.spanFrom(start, end), buffer.toString(), ScalarStyle.PLAIN);
  }
  ...
  bool _isBlankOrEndAt(int offset) {
    var char = _scanner.peekChar(offset);
    return char == null || char == SP || char == TAB || char == CR || char == LF;
  }
  ...
  bool _isStandardCharacter(int char) =>
      (char >= 0x0020 && char <= 0x007E) || (char >= 0x00A0 && char <= 0xD7FF) || (char >= 0xE000 && char <= 0xFFFD);
...
class _SimpleKey {
  ...
  _SimpleKey(this.tokenNumber, this.line, this.column, this.location, {required this.required});
}
