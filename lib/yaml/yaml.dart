...
export 'style.dart';
export 'utils.dart' show YamlWarningCallback, yamlWarningCallback;
export 'yaml_document.dart';
export 'yaml_exception.dart';
export 'yaml_node.dart' hide setSpan;
...
dynamic loadYaml(String yaml, {Uri? sourceUrl, bool recover = false, ErrorListener? errorListener}) =>
    loadYamlNode(yaml, sourceUrl: sourceUrl, recover: recover, errorListener: errorListener).value;
...
YamlNode loadYamlNode(String yaml, {Uri? sourceUrl, bool recover = false, ErrorListener? errorListener}) =>
    loadYamlDocument(yaml, sourceUrl: sourceUrl, recover: recover, errorListener: errorListener).contents;
...
YamlDocument loadYamlDocument(String yaml, {Uri? sourceUrl, bool recover = false, ErrorListener? errorListener}) {
  var loader = Loader(yaml, sourceUrl: sourceUrl, recover: recover, errorListener: errorListener);
  var document = loader.load();
  if (document == null) {
    return YamlDocument.internal(YamlScalar.internalWithSpan(null, loader.span), loader.span, null, const []);
  }
...
YamlList loadYamlStream(String yaml, {Uri? sourceUrl}) {
  ...
  return YamlList.internal(documents.map((YamlDocument document) => document.contents).toList(), loader.span, CollectionStyle.ANY);
}
